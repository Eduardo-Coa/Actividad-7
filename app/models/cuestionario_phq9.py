...
class Respuesta:
    ...
    def __post_init__(self) -> None:
        if not (1 <= self.numero_pregunta <= NUM_ITEMS_PHQ9):
            raise PuntajeInvalidoError(
                self.numero_pregunta, (1, NUM_ITEMS_PHQ9)
            )
        if not (
            PUNTAJE_MINIMO_ITEM
            ...
            raise PuntajeInvalidoError(
                self.valor_respuesta,
                (PUNTAJE_MINIMO_ITEM, PUNTAJE_MAXIMO_ITEM),
            )
    ...
    def to_dict(self) -> dict:
        ...
        Returns:
            Diccionario con numero_pregunta y valor_respuesta.
        """
        return {
    ...
    def from_dict(cls, datos: dict) -> Respuesta:
        """Deserializa una respuesta desde un diccionario.

        Args:
            datos: diccionario con numero_pregunta y valor_respuesta.
...
class CuestionarioPHQ9:
    ...
    def _validar(self) -> None:
        if not self.id or not self.id.strip():
            raise FechaInvalidaError(
                "El ID del cuestionario no puede estar vacío."
            )
        if not self.codigo_estudiante or not self.codigo_estudiante.strip():
            raise FechaInvalidaError(
                "El código del estudiante no puede estar vacío."
            )
        if not isinstance(self.fecha_aplicacion, datetime):
            raise FechaInvalidaError(
                "La fecha de aplicación debe ser un datetime."
            )
        ...
        if self.respuestas:
            ...
            for valor in self.respuestas:
                if not (
                    PUNTAJE_MINIMO_ITEM <= valor <= PUNTAJE_MAXIMO_ITEM
                ):
                    raise PuntajeInvalidoError(
                        valor,
                        (PUNTAJE_MINIMO_ITEM, PUNTAJE_MAXIMO_ITEM),
    ...
    def to_dict(self) -> dict:
        """Serializa el cuestionario a diccionario JSON-compatible.

        Returns:
            Diccionario con todos los campos del cuestionario.
        """
        return {
            "id": self.id,
    ...
    def from_dict(cls, datos: dict) -> CuestionarioPHQ9:
        """Deserializa un cuestionario desde un diccionario.

        Args:
            datos: diccionario con los campos del cuestionario.

        Returns:
            Instancia de CuestionarioPHQ9.
        """
        return cls(
            id=datos["id"],
